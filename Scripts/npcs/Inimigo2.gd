...
func post_ready():
	get_node("/root/level7/player/feet_sensor").connect("dialog_inimigo2", self, "on_enter")
	get_node("/root/level7/player/feet_sensor").connect("dialog_inimigo2_exited", self, "_end_dialog")
...
func on_enter():
	...
	if player_vars.win_inimigo2 == false:
		
		player_vars.win_inimigo2 = true
